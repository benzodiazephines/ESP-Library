			...
			if self.Player and self.Player:FindFirstChildOfClass("Humanoid") then
				if self.Player and CharTable[self.Player] then
					if self.Player and CharTable[self.Player] then
						...
						local LA_Height = CharTable[self.Player]["left arm"].Size.Y / 2 - .2
						local LUA = WorldToViewportPoint(cam, (CharTable[self.Player]["left arm"].CFrame * CFrame.new(0, LA_Height, 0)).p)
						local LLA = WorldToViewportPoint(cam, (CharTable[self.Player]["left arm"].CFrame * CFrame.new(0, -LA_Height, 0)).p)
						local RA_Height = CharTable[self.Player]["right arm"].Size.Y / 2 - .2
						local RUA = WorldToViewportPoint(cam, (CharTable[self.Player]["right arm"].CFrame * CFrame.new(0, RA_Height, 0)).p)
						local RLA = WorldToViewportPoint(cam, (CharTable[self.Player]["right arm"].CFrame * CFrame.new(0, -RA_Height, 0)).p)
						local LL_Height = CharTable[self.Player]["left leg"].Size.Y / 2 - .2
						local LUL = WorldToViewportPoint(cam, (CharTable[self.Player]["left leg"].CFrame * CFrame.new(0, LL_Height, 0)).p)
						local LLL = WorldToViewportPoint(cam, (CharTable[self.Player]["left leg"].CFrame * CFrame.new(0, -LL_Height, 0)).p)
						local RL_Height = CharTable[self.Player]["right leg"].Size.Y / 2 - .2
						local RUL = WorldToViewportPoint(cam, (CharTable[self.Player]["right leg"].CFrame * CFrame.new(0, RL_Height, 0)).p)
						local RLL = WorldToViewportPoint(cam, (CharTable[self.Player]["right leg"].CFrame * CFrame.new(0, -RL_Height, 0)).p)
