...
function boxBase:Update()
    ...
    if ESP.Health then
        ...
        if Vis8 then
            if self.Player and self.Player:FindFirstChildWhichIsA("Humanoid") then
                ...
                local offset = nil
                offset = self.Player.Humanoid.Health / self.Player.Humanoid.MaxHealth * b
                ...
                self.Components.Health2.Color = r:lerp(g, self.Player.Humanoid.Health / self.Player.Humanoid.MaxHealth)
            end
        else
    ...
    if ESP.Items then
        ...
        if Vis9 then        
	        if self.Player and self.Player:FindFirstChildWhichIsA("Tool") then
            	self.Components.Items.Text = tostring(self.Player:FindFirstChildWhichIsA("Tool").Name)
